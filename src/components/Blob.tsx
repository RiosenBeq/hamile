...
function BlobImpl({ style, variant = 'cream' }: Props) {
  // Per-instance gradient ids. SVG <defs> in React Native usually scope to the
  // owning <Svg>, but on web they collide across components — using a stable
  // unique id keeps both renderers honest.
...
function IlloImpl({ label, hue = 'rose', size = 56 }: { label?: string; hue?: Hue; size?: number }) {
  ...
  // Sanitise the label to be safe inside an SVG id; spaces, punctuation and
  // unicode would otherwise produce malformed `url(#...)` references.
